...
export default function TranslatorPage() {
  ...
  const handleLoad = async (stepId) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        useNotificationStore.getState().error(data.error || "File not found");
      }
    } catch (e) {
  ...
  const handleToOpenAI = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!data.success) { useNotificationStore.getState().error(data.error); return; }
      const str = JSON.stringify(data.result.body, null, 2);
      setContent(3, str);
      openNext(3);
    } catch (e) { useNotificationStore.getState().error(e.message); }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!provider || !model) {
        useNotificationStore.getState().error("Missing provider or model. Please run step 1 first to detect them.");
        return;
      }
      ...
      if (!res.ok) {
        ...
        useNotificationStore.getState().error(err.error || "Send failed");
        return;
      }
